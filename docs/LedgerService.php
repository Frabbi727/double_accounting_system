...
class LedgerService
{
    ...
    public function reverse(JournalEntry $original, string $reason): JournalEntry
    {
        if ($original->reversed_by_id !== null) {
            throw new \RuntimeException('এই এন্ট্রি আগেই রিভার্স করা হয়েছে।');
        }
        ...
        return DB::transaction(function () use ($original, $mirroredLines, $reason) {
            ...
            $reversal = $this->post(
                ...
                description:   "রিভার্সাল: {$original->description} — {$reason}",
                lines:         $mirroredLines,
            );
    ...
    private function assertBalanced(array $lines): void
    {
        ...
        if ($debit < self::EPSILON) {
            throw new \InvalidArgumentException('শূন্য পরিমাণের জার্নাল এন্ট্রি পোস্ট করা যাবে না।');
        }
    }
    ...
    private function assertNoZeroLines(array $lines): void
    {
        foreach ($lines as $i => $line) {
            ...
            if ($d > 0 && $c > 0) {
                throw new \InvalidArgumentException("লাইন #{$i}: একই লাইনে ডেবিট ও ক্রেডিট দুটোই থাকতে পারে না।");
            }
            if ($d <= 0 && $c <= 0) {
                throw new \InvalidArgumentException("লাইন #{$i}: ডেবিট বা ক্রেডিট কোনো একটি থাকতে হবে।");
            }
            if ($d < 0 || $c < 0) {
                throw new \InvalidArgumentException("লাইন #{$i}: ঋণাত্মক পরিমাণ দেওয়া যাবে না।");
            }
        }
    }
}
