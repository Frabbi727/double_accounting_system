<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800">{{ __('ui.supplier.details') }}</h2>
            <a href="{{ route('suppliers.index') }}" class="text-sm text-gray-500">{{ __('ui.common.cancel') }}</a>
        </div>
    </x-slot>
    ...
    <div class="py-8 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-lg shadow p-6 mb-4 flex flex-wrap items-start justify-between gap-4">
            <div class="space-y-1">
                ...
                @if ($record->phone)
                    <div class="text-sm text-gray-500">{{ __('ui.supplier.phone') }}: {{ $record->phone }}</div>
                @endif
                @if ($record->address)
                    <div class="text-sm text-gray-500">{{ __('ui.supplier.address') }}: {{ $record->address }}</div>
                @endif
            </div>
            <div class="text-right">
                ...
                @can('payment.manage')
                    <a href="{{ route('payments.create', ['direction' => 'made', 'party_id' => $record->id]) }}"
