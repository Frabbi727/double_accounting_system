<x-opening-wizard
    ...
    :title="__('ui.opening.wizard.customers_title')"
    :help="__('ui.opening.wizard.customers_help')">

    <form method="POST" action="{{ route('opening.setup.customers') }}" class="grid grid-cols-1 sm:grid-cols-12 gap-3 items-end">
        @csrf
        ...
        <div class="sm:col-span-2">
            <label class="text-xs font-medium text-gray-600">{{ __('ui.opening.wizard.due_amount') }}</label>
    ...
    @include('shop.opening.setup._added_list', ['rows' => $rows, 'emptyKey' => 'customers_empty'])
</x-opening-wizard>
